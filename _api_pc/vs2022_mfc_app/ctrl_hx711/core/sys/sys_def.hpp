...
namespace ap_core
{
	...
	/*
6자리로 구성되며  object type [xxx] + 정의된 error no [xxx]
*/
	typedef int errcode;
	 ...
	 */
	constexpr auto APP_PERIPH_BASE = 0x00000000UL;// 증가 시 0x00100000UL~;
	...
	struct IObj
	{
		...
		{		
			...
			va_start(args, fmt);
			std::string message{};
			message.reserve(1024); // 버퍼 크기를 조정
			// 버퍼의 크기를 동적으로 결정
			int size = vsnprintf(nullptr, 0, fmt, args);
			message.resize(size);
			// 버퍼를 생성하고 vsnprintf 함수를 사용하여 문자열을 작성
		...
		/// <summary>
		 ...
		 /// </summary>
		 /// <param name="err_code">object type [xxx] + 정의된 error no [xxx]</param>
