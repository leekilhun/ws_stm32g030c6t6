 ...
 *
 *
 * \author lee kil hun gns.lee2@samsung.com
 * \date   March 2024
 *********************************************************************/
#pragma once
...
namespace tim
{
  ...
  {
  public:
    timer() : isRunning(false), totalDuration(0) {}
    ~timer() {
      stop();
    }

    template<typename Callable, typename... Args>
    void start(std::chrono::milliseconds interval_ms, Callable&& func, Args&&... args) {
      if (isRunning.load()) {
        return; // 이미 실행 중인 타이머가 있다면 추가 실행을 방지
      }
      isRunning.store(true);
      thread = std::thread([this, interval_ms, func, args...]() {
        while (this->isRunning.load()) {
          auto start = std::chrono::high_resolution_clock::now();

          std::invoke(func, args...); // 변경
...
/*
*       주기에 따른 등록 함수 호출 사용 예

struct led {
    led() = default;
    void blink(const std::string& name, int interval_ms) {
        std::cout << name << " pass time: " << interval_ms << "ms" << std::endl;
    }
};
...
struct btn {
    ...
    void read(const std::string& name) {
        std::cout << name << " status: " << state << std::endl;
    }
};

struct uart {
  int UartCallback(void* arg1, void* arg2) {
    // 콜백 함수 구현
    std::cout<< "name" << std::string{(char*)arg1}<< "durane time" << *(int*)arg2<<std::endl;
    return 0;
  }
};
...
public:
  int UartCallback(void* arg1, void* arg2) {
    // 콜백 함수 구현
    std::cout << "name: " << std::string{ (char*)arg1 } << ", duration time: " << *(int*)arg2 << "ms" << std::endl;
    return 0;
  }
};

timer blinkled_timer{};

void set_timer()
{
  std::shared_ptr<blink_test> led_ptr{};
  constexpr int               def_duration = 200;          // 이 값을 `void*`로 캐스팅하기 위해 변수로 만듭니다.
  led_ptr = std::make_shared<blink_test>();
  led_ptr->ledname = "test led";
  led_ptr->duration = def_duration; //

  blinkled_timer.start(std::chrono::milliseconds(def_duration), [led_ptr]() {
    led_ptr->UartCallback((void*)led_ptr->ledname.c_str(), (void*)&led_ptr->duration);
    });
}
void init_timer()
{
...
int main() {
    timer blinkTimer, btnTimer;
    led myLed;
    btn myBtn;

    // 수정: blink 함수는 void를 반환하므로 start의 호출을 수정합니다.
    blinkTimer.start(std::chrono::milliseconds(500), std::bind(&led::blink, &myLed, "Blinking LED", 500));

    // 1초 간격으로 read 함수 호출
    btnTimer.start(std::chrono::milliseconds(1000), std::bind(&btn::read, &myBtn, "Button"));


  timer uartTimer;
  uart myUart;
  std::string uartName = "test led";
  int duration = 500; // 이 값을 `void*`로 캐스팅하기 위해 변수로 만듭니다.
  uartTimer.start(std::chrono::milliseconds(500), [&myUart, uartName, duration]() {
    myUart.UartCallback((void*)uartName.c_str(), (void*)&duration);
    });
...
출력
nametest leddurane time500
Blinking LED pass time: 500ms
name: test led, duration time: 200ms
name: test led, duration time: 200ms
Blinking LED pass time: 500ms
nametest leddurane time500
name: test led, duration time: 200ms
