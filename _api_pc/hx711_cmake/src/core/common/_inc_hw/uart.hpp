  ...
  ****************************************************/
    bool write_log(TinyC::Log::level level, ap_core::log_info_t& log, const char* fmt, ...)
    {
      if (fmt)
      {
        ...
        std::string str(1024, '\0');  // 충분한 크기의 문자열을 생성하고 초기화합니다.
        int needed = vsnprintf(&str[0], str.size(), fmt, args);  // vsnprintf를 사용하여 문자열에 직접 포맷된 데이터를 씁니다.
        if (needed < 0)
        {
          // 오류 처리
          LOG_PRINT("va_list 함수의 처리할 내용이 없습니다.");
          return false;
        }

        // 실제로 필요한 크기를 초과할 수 있으므로, 필요한 경우 문자열의 크기를 조정합니다.
        if (needed >= str.size())
        {
          str.resize(needed + 1);
          va_start(args, fmt);  // va_list를 다시 초기화합니다.
          vsnprintf(&str[0], str.size(), fmt, args);  // 다시 쓰기를 시도합니다.
        }
        else
        {
          str.resize(needed);  // 실제 사용된 크기에 맞게 문자열의 크기를 조정합니다.
        }
        va_end(args);
