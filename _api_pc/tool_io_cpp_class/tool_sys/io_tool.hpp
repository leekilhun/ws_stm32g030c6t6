...
namespace toolsys
{
  ...
  */
  ...
  public:
    ...
    mcu_io_align_12_t m_toolData{};

  public:
    IOTool() : m_is_init{}, m_cfg{}, m_errorReg{}, m_optionReg{}, m_stateReg{}, m_inReg{}, m_outReg{}, m_packetData{}, m_trd{}, m_stopThread{}, m_cb{}, m_mutex{}, m_msgTime{}, m_toolsInfo{}, m_toolData{}
    {
    }
  ...
  private:
    ...
    inline void processCplt()
    {
      ...
      case RX_TYPE::RX_TOOL_DATA:
        m_toolData = *((mcu_io_align_12_t *)m_packetData.data);
  ...
  public:
    inline errno_t Init(const cfg_t &cfg)
    {
      ...
      m_cfg.ptr_uart_comm->registerCallback(std::bind(&IOTool::UartCallback, this, std::placeholders::_1, std::placeholders::_2));
    ...
    inline errno_t GetExtIOData()
    {
      std::vector<uint8_t> datas{};

      datas.emplace_back(LoBYTE(TX_CTRL_READ_EXT_IO));
      datas.emplace_back(HiBYTE(TX_CTRL_READ_EXT_IO));
    ...
    inline int testCB_func(int argc, char *argv[])
    {
      ...
      case arg_cnt_1:
      {
        if (argv[1])
        {
          ...
          else if (std::string(argv[1]).compare("get_ext_io") == 0)
          {
            if (this->GetExtIOData() == ERROR_SUCCESS)
            {
              std::cout << "tools get_ext_io [" << this << "] success " << std::endl;
              ret = true;
            }
            else
              std::cout << "tools get_ext_io fail" << std::endl;
          }
        }
      }
      break;
      ...
      std::cout << "tools get_ext_io" << std::endl;
      std::cout << "tools on_io [idx 0 ~ 31]" << std::endl;
      std::cout << "tools io_out [idx] [on/off]" << std::endl;
