...
{
  // iicLoadcell::cfg_t cfg{};
  // cfg.i2c_ch = _DEF_I2C1;

  // if (iic_loadcell.Init(cfg) != ERROR_SUCCESS)
  //   LOG_PRINT("iic_loadcell Init Failed!");
}
...
void apMain(void)
{
  ...
  while (1)
  {
    if (millis() - pre_time >= 1'000)
    {
      ...
      // if (iic_loadcell.is_opened == false)
      // {
      //   //i2cReset(iic_loadcell.m_cfg.i2c_ch);
      // }
...
void apISR_10ms(void *arg)
{
  ...
  //int32_t weight = 0;
  //weight = iic_loadcell.get_weight_int();
