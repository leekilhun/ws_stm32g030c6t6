...
bool i2cReadData(uint8_t ch, uint16_t dev_addr, uint8_t *p_data, uint32_t length, uint32_t timeout)
{
  ...
  LOCK_BEGIN(ch);
#ifdef _USE_HW_I2C_1_DMA_RX
  i2c_ret = (HAL_I2C_Master_Receive_DMA(p_handle, (uint16_t)(dev_addr << 1), p_data, length) != HAL_OK);
#else
...
#ifdef _USE_HW_I2C_1_DMA_TX
  i2c_ret = (HAL_I2C_Master_Transmit_DMA(p_handle, (uint16_t)(dev_addr << 1), p_data, length) != HAL_OK);
#else
