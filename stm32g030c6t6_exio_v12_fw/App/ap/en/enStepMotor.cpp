...
bool enStepMotor::ProcessZeroSet()
{
  ...
  case STEP_CHECK_DISABLE_WAIT:
  {
    ...
    if (m_cfg.ptr_io->IsOff(m_cfg.io_input_idx_zero) && m_cfg.ptr_io->IsOff(m_cfg.io_input_idx_zero_ex))
    {
      m_step.SetStep(STEP_CHECK_DISABLE_END);
      Enable();
    }
...
void enStepMotor::ISR_axis(void *obj, void *w_param, void *l_param)
{
  ...
  if (pM->m_status.moving)
  {
    if (pM->accel_index != 0)
    {
      ...
      // max speed
      else if ((pM->step_count >= pM->accel_index) && (pM->step_count <= pM->accel_index + pM->vel_index))
      {
  ...
  if (pM->move_done == true)
    remaining_flag &= ~(1 << pM->m_cfg.obj_idx); // Off  motor timer_cal
};
