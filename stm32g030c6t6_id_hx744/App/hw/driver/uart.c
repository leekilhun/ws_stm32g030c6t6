...
uint32_t uartWrite(uint8_t ch, uint8_t *p_data, uint32_t length)
{
  ...
  if (ch >= UART_MAX_CH)
    return 0;
  UART_HandleTypeDef * p_huart = uart_tbl[ch].p_huart;
  ...
  if (is_dma)
  {
    uint32_t pre_time;
    pre_time = millis();
    while (p_huart->gState != HAL_UART_STATE_READY)
    {
    ...
    if (p_huart->gState == HAL_UART_STATE_READY)
    {
      memset(uart_tbl[ch].p_tx_buf, 0, UART_BUF_LENGTH);
      memcpy(uart_tbl[ch].p_tx_buf, p_data, length);
      hal_ret = HAL_UART_Transmit_DMA(p_huart, uart_tbl[ch].p_tx_buf, length);
    }
  }
  else
  {
    hal_ret = HAL_UART_Transmit(p_huart, p_data, length, 100);
  }
