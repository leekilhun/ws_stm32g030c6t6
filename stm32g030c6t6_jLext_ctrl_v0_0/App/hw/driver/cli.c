...
#if defined(_USE_HW_CLI) && defined(_USE_HW_UART)
...
bool cliMain(void)
{
  ...
  {
    uint8_t data = uartRead(cli_node.ch);
    cliUpdate(&cli_node, data);
    //LOG_PRINT("cli main receive data [%d]", data);
  }
...
bool cliUpdate(cli_t *p_cli, uint8_t rx_data)
{
  ...
  line = &p_cli->line;
 //LOG_PRINT("cliUpdate receive data [%d]", rx_data);
