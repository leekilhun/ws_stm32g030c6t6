...
bool ssd1306Reset(void)
{
  ...
  ret = i2cBegin(i2c_ch, I2C_FREQ_100KHz);

  if (ret != true)
  {
    LOG_PRINT("[NG] i2cBegin");
    return false;
  }

  if (i2cIsDeviceReady(i2c_ch, i2c_dev) != true)
  {
    LOG_PRINT("[NG] i2cIsDeviceReady");
    return false;
  }

  /* Init LCD */
  if (ssd1306WriteCmd(0xAE) != true) // display off
    LOG_PRINT("[NG] write reg  display off");

  if(ssd1306WriteCmd(0x20) != true) //Set Memory Addressing Mode
    LOG_PRINT("[NG] write reg  Set Memory Addressing Mode");

  if(ssd1306WriteCmd(0x10) != true) //00,Horizontal Addressing Mode;01,Vertical Addressing Mode;10,Page Addressing Mode (RESET);11,Invalid
    LOG_PRINT("[NG] write reg  Set Page Start Address for Page Addressing Mode,0-7");
