...
const can_baud_cfg_t can_baud_cfg_80m_normal[] =
    {
        {40, 16, 13, 2}, // 100K, 87.5%
        {32, 16, 13, 2}, // 125K, 87.5%
        {16, 16, 13, 2}, // 250K, 87.5%
        { 8, 16, 13, 2}, // 500K, 87.5%
        { 4, 16, 13, 2}, // 1M,   87.5%
};
...
bool canInit(void)
{
  ...
  for(i = 0; i < CAN_MAX_CH; i++)
  {
    ...
    if (qbufferCreateBySize(&can_tbl[i].q_msg, (uint8_t *)&can_tbl[i].can_msg[0], sizeof(can_msg_t), CAN_MSG_RX_BUF_MAX) == false)
      LOG_PRINT("qbufferCreateBySize() fail");
  }

  can_tbl[_DEF_CAN1].p_hfdcan = & hfdcan2;

 LOG_PRINT("[OK] can Init()");
...
bool canOpen(uint8_t ch, CanMode_t mode, CanFrame_t frame, CanBaud_t baud, CanBaud_t baud_data)
{
  ...
  {
    LOG_PRINT("HAL_FDCAN_Init() failed");
    return false;
  }
 ...
 {
    LOG_PRINT("HAL_FDCAN_ConfigGlobalFilter() failed");
    return false;
  }
  if (HAL_FDCAN_ActivateNotification(p_can, can_tbl[ch].enable_int, 0) != HAL_OK)
  {
    LOG_PRINT("HAL_FDCAN_ActivateNotification() failed");
    return false;
  }
  ...
  {
    LOG_PRINT("HAL_FDCAN_ConfigTxDelayCompensation() failed");
    return false;
  }
  if (HAL_FDCAN_EnableTxDelayCompensation(p_can) != HAL_OK)
  {
    LOG_PRINT("HAL_FDCAN_EnableTxDelayCompensation() failed");
    return false;
  }

  if (HAL_FDCAN_Start(p_can) != HAL_OK)
  {
    LOG_PRINT("HAL_FDCAN_Start() failed");
    return false;
  }
...
void canClose(uint8_t ch)
{
  ...
  {
    HAL_FDCAN_DeInit(can_tbl[ch].p_hfdcan);
  }
  return;
}
...
uint32_t canGetState(uint8_t ch)
{
  ...
  return HAL_FDCAN_GetState(can_tbl[ch].p_hfdcan);
}
...
void cliCan(cli_args_t *args)
{
  ...
  if (args->argc == 2 && args->isStr(0, "open") && args->isStr(1, "test"))
  {
    ...
    can_ret = canOpen(_DEF_CAN1, CAN_LOOPBACK, CAN_FD_BRS, CAN_1M, CAN_1M);
    cliPrintf("canOpen() : %s\n", can_ret ? "True":"False");
  ...
  if (args->argc == 6 && args->isStr(0, "open"))
  {
    ...
    const char *baud_str[]  = {"CAN_100K", "CAN_125K", "CAN_250K", "CAN_500K", "CAN_1M", "CAN_2M", "CAN_4M"};
